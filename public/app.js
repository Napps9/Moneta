...
function renderAccountRow(account, { showInstitution = false } = {}) {
  ...
  if (account.balance && account.balance.alt && account.balance.pending) {
    const { alt, pending, currency } = account.balance;
    meta.push(el('span', { text: alt.cleared ? `${formatMoney(alt.amount, currency)} once ${plural(pending.count, 'pending payment')} clear` : `${formatMoney(alt.amount, currency)} before ${plural(pending.count, 'pending payment')}`, title: `Pending: ${formatMoney(pending.net, currency)}` }));
  }
...
function renderActivityHead(account, sheet) {
  ...
  if (balance && balance.alt && balance.pending) {
    parts.push(balance.alt.cleared ? `${formatMoney(balance.alt.amount, balance.currency)} once ${plural(balance.pending.count, 'pending payment')} clear` : `${formatMoney(balance.alt.amount, balance.currency)} before ${plural(balance.pending.count, 'pending payment')}`);
  }
...
function openEditor(account) {
  ...
  const pendingMode = account.settings && (account.settings.pending === 'exclude' || account.settings.pending === 'booked') ? account.settings.pending : 'include';
  editorPending.value = pendingMode;
  ...
  const pendingText = pending && pending.count ? `${plural(pending.count, 'pending payment')} (${formatMoney(pending.net, currency)})` : 'nothing pending';
  editorPendingNote.textContent =
    pending && reportedNow != null
      ? pendingMode === 'booked'
        ? `Lunch Flow reports ${formatMoney(reportedNow, currency)}, taken as the booked balance with ${pendingText} still to come off. If its Balance tab for this account is on interimAvailable, choose the first option instead.`
        : pending.count
          ? `Lunch Flow reports ${formatMoney(reportedNow, currency)} with ${pendingText} already taken off; before them it is ${formatMoney(reportedNow - pending.net, currency)}. Pick whichever matches your bank app, or set interimBooked on the account’s Balance tab in Lunch Flow and choose the last option.`
          : `Lunch Flow reports ${formatMoney(reportedNow, currency)} and nothing is pending right now, so the choices read the same today.`
...
async function submitEditor(event) {
  ...
  if (editorPending.value === 'exclude' || editorPending.value === 'booked') entry.pending = editorPending.value;
  if (Object.keys(entry).length > 0) next.accounts[id] = entry;
  else delete next.accounts[id];
