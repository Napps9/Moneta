...
Forecasts come in two modes, switched with **Auto / Budget** above the sheet and remembered per account. In Auto, outgoings and transfers run at the average of the last six complete months and income repeats its latest complete month, so a salary or interest payment carries on as it is. In Budget, nothing is guessed: only the amounts you set count, and everything else is nothing until you set it. In either mode, tap a forecast to set your own amount for that line in that account, or go back to automatic. The projected balance starts from the balance now and first adds what is still expected before the current month ends: each line's forecast less what has already happened, never below zero.
