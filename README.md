...
Next to every row name a small bar chart shows the months in view and an arrow says whether the row is rising, falling or steady over the complete months (the current month is left out, since it is only part way through). Green is the direction you want, income up or spending down, red is the other way, and hovering or tapping the arrow gives the last two complete months side by side. The **Amounts / Change** switch above the sheet turns every figure into the difference from the month before, coloured the same way.
