...
Forecasts come in two modes, switched with **Auto / Budget** above the sheet and remembered per account. In Auto, outgoings and transfers run at the average of the last six complete months and income repeats its latest complete month, so a salary or interest payment carries on as it is. In Budget, nothing is guessed: only the amounts you set count, and everything else is nothing until you set it. In either mode, tap a forecast to set your own amount for that line in that account, either for **every month** or for **that month only**, so a repayment that tails off or a one-off next month can be put in exactly. A month's own amount beats the every-month one; a pencil marks an every-month amount and a diamond a month's own. "Use automatic" takes a line back to the automatic figure. The projected balance starts from the balance now and first adds what is still expected before the current month ends: each line's forecast less what has already happened, never below zero.
