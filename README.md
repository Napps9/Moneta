...
The sheet is a window of six months that slides through time with the arrows: back into history, and forward past today into the forecasts, up to a year ahead. It opens on the last three months plus the next three. Months up to now are actuals; the current month appears twice, "so far" beside its forecast for the whole month, so you can see the real figures against the expected ones; later months are forecasts, set off by a double rule, with the projected balance at each month end in the header. Six complete months are always fetched behind the view for the trends and the forecasts.

Forecasts come in two modes, switched with **Auto / Budget** above the sheet and remembered per account. In Auto, each line learns its own forecast from the twelve complete months behind the sheet, fetched whatever months are in view. Several ways of forecasting are tried against the last six of those months as if each were still to come, using only the months before it: the average of the last six months, the same as last month, a typical month with one-offs left out, an average weighted towards recent months, and the average over the year. The one that would have been closest is used for the months ahead, so rent and a salary settle on "same as last month", lumpy spending on a typical month, and a line that has shifted on recent months. Until a line has enough history, spending and transfers use the six-month average and income repeats its latest month, and a line only moves off that default when another way is clearly better. Each month that passes adds another test, so the forecasts improve on their own and nothing is stored. The note under the sheet says how far off the forecasts would have been on total outgoings and income, and tapping a forecast says how that line was worked out and how an amount you set would have fared. In Budget, nothing is guessed: only the amounts you set count, and everything else is nothing until you set it. In either mode, tap a forecast to set your own amount for that line in that account, either for **every month** or for **that month only**, so a repayment that tails off or a one-off next month can be put in exactly. A month's own amount beats the every-month one; a pencil marks an every-month amount and a diamond a month's own. "Use automatic" takes a line back to the automatic figure. The projected balance starts from the balance now and first adds what is still expected before the current month ends: each line's forecast less what has already happened, never below zero.

The current month's forecast is never less than what has already happened. When a line's spending or income so far passes its forecast, automatic or set by you, that month's forecast rises to the actual, marked with an arrow and shown in amber, and totals follow. The planned figure is kept in the cell's tooltip and the dialog, your set amounts stay stored as they were, and the months ahead keep their forecasts. A note under the sheet says how many lines are over.
...
Lunch Flow reports a balance with pending card payments already taken off, while most bank apps show the balance before them, so the two can differ by exactly the pending amount. The app fetches the last month of transactions for every account along with its balance, so it knows what is pending and when the newest transaction was; each row and the sheet head say "£702.98 before 2 pending payments" and "Latest transaction 24 Sep". Open the account with its pencil and set **Pending payments** to "show it before pending" to show the balance the way your bank app does; the sheet then moves the balance only for booked transactions and lets the pending ones through in the forecast. Lunch Flow can also send the booked balance itself (the account's Balance tab in Lunch Flow, interimBooked rather than interimAvailable); choose "Lunch Flow sends the booked balance" here so nothing is put back on twice. Lunch Flow warns that a pending purchase can post as a brand-new record with a different id, so a pending transaction whose posted twin has arrived (same amount and merchant, on the same day or up to a week later) is dropped and nothing counts twice.
...
```json
{
  ...
  "budgets": {
    "out:groceries": 200,
    "in:salary": 4180,
    "out:hsbcloan": { "each": 0, "months": { "2026-10": 83.41, "2026-11": 83.41 } }
  }
}
```

Each line is matched to the Lunch Flow transaction with the same month, side and amount that no other line has claimed, over the last twelve months Lunch Flow has, and takes that category as a per-transaction choice. A merchant matched the same way at least twice becomes a rule, and a rule saved earlier that the matches contradict is removed. Lines left over in a month that together add up to a transaction nothing else matched become a split of it, with lines in the same category merged into one part: rent and bills paid to one person as a single transfer, which the spreadsheet records line by line, land as that transfer split the same way. When several lines share an amount (three £50 bills, say), the split takes the categories that payment has in its other months too, and leaves a line whose category a transaction is already filed under with that transaction. The preview says what would change, lists the splits and the lines that found no transaction, and nothing is saved until you apply. `budgets`, keyed like the forecast rows, can be applied at the same time, which also switches the account to Budget mode: a plain number is the amount for every month, and `{ "each", "months" }` gives some months an amount of their own, exactly as the forecast dialog does. Lines for a group rather than a category, or with a refund inside an outgoing line, are reported and left alone.
