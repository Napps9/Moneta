...
Lunch Flow reports a balance with pending card payments already taken off, while most bank apps show the balance before them, so the two can differ by exactly the pending amount. The app fetches the last month of transactions for every account along with its balance, so it knows what is pending and when the newest transaction was; each row and the sheet head say "£702.98 before 2 pending payments" and "Latest transaction 24 Sep". Open the account with its pencil and set **Pending payments** to "Not yet taken off" to show the balance the way your bank app does; the sheet then moves the balance only for booked transactions and lets the pending ones through in the forecast. Lunch Flow warns that a pending purchase can post as a brand-new record with a different id, so a pending transaction whose posted twin has arrived (same amount and merchant, on the same day or up to a week later) is dropped and nothing counts twice.
