...
The last three months are shown as actuals; six complete months are fetched behind them for the trends and the forecasts. To the right sits a second table, **Projections**: the next three months, with the projected balance at each month end in its header. Outgoings and transfers run at the average of the last six complete months; income repeats its latest complete month, so a salary or interest payment carries on as it is. Tap any forecast to set your own amount for that line in that account (a budget), or go back to automatic. The projected balance starts from the balance now and first adds what is still expected before the current month ends: each line's forecast less what has already happened, never below zero.
